...
def extract_suffix(csv_path: Path) -> str:
    stem = csv_path.stem
    m = re.match(r"^postgresql_array_run\d+_(.+)$", stem)
    if m:
        return m.group(1)
    return "unknown"
    ...
    for ax, operation in zip(axes, operations_to_plot):
        ...
        for phase in phase_order:
            ...
            if band_df is not None:
                phase_band = band_df[(band_df["Operation"] == operation) & (band_df["Phase"] == phase)].sort_values("Epoch")
...
def main() -> None:
    files = sorted(DATA_DIR.glob("postgresql_array_*.csv"))
    if not files:
        raise RuntimeError("No PostgreSQL array files found in DATA/")

    base_out_dir = RESULTS_DIR / "postgresql_array"
    ...
    for suffix, suffix_files in sorted(files_by_suffix.items()):
        ...
        for metric_col, folder_name, metric_label in METRICS:
            metric_dir = base_out_dir / f"postgresql_array_epoch_{folder_name}"
