...
CREATE TABLE usertable (
	...
	FIELD0 JSONB, FIELD1 JSONB,
	FIELD2 JSONB, FIELD3 JSONB,
	FIELD4 JSONB, FIELD5 JSONB,
	FIELD6 JSONB, FIELD7 JSONB,
	FIELD8 JSONB, FIELD9 JSONB
);
```
...
* The fields are stored as JSON arrays inside PostgreSQL `JSONB` columns.

### JDBC Array JSON Binding (jdbc-array-json)
This binding stores each field as a JSON array. Each value is split by a delimiter into array elements and joined back on reads.
...
bin/ycsb run jdbc-array-json -P workloads/workloada-extend -P db.properties
```
...
```sh
java -cp YCSB_HOME/jdbc-array-json/lib/jdbc-array-json-binding-0.4.0.jar:mysql-connector-java-5.1.37-bin.jar site.ycsb.db.JdbcDBCreateTable -P db.properties -n usertable
```
...
```sh
bin/ycsb load jdbc-array-json -P workloads/workloada-extend -P db.properties -cp postgresql.jar
```

Then, you can run the workload:

```sh
bin/ycsb run jdbc-array-json -P workloads/workloada-extend -P db.properties -cp postgresql.jar
```
